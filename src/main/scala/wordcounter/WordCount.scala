...
object WordCount {
  type WordMap = TMap[Word, TMap[EventType, WordData]]

  def server(wordMap: WordMap): HttpApp[Any, Nothing] = Http.collectZIO[Request] {
    case Method.GET -> !! / "word" / word =>
      STM.atomically {
        for {
          currentCount <- wordMap.get(Word(word))
          wc           <- currentCount match {
                            case None => ZSTM.succeed(List.empty)
  ...
  def insertInWordMap(newData: WordData, wordMap: WordMap): UIO[WordData] =
    STM.atomically {
      wordMap
        .get(newData.word)
        .flatMap {
          case None => TMap.make((newData.eventType, newData)).flatMap(wordMap.put(newData.word, _))
          case Some(eventMap) =>
